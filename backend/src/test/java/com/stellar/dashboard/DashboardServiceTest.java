 ...
 * <p>任务/TTS 统计走 {@link AiTaskService} 聚合方法（taskStatTotals/taskStatRecent/ttsStat），
...
class DashboardServiceTest {
    ...
    private void stubEmptyTaskAggregates() {
        lenient().when(aiTaskService.taskStatTotals(anyString(), anyString())).thenReturn(taskTotals(0, 0, 0, 0));
        lenient().when(aiTaskService.taskStatRecent(anyString(), any(), any(), any())).thenReturn(taskRecent(0, 0, 0));
        lenient().when(aiTaskService.ttsStat(any())).thenReturn(ttsStat(0, 0, 0));
        lenient().when(fileMapper.selectList(any(Wrapper.class))).thenReturn(List.of());
    }

    @Test
    void stats_正常聚合所有模块() {
        DashboardService service = new DashboardService(aiUsageService, aiTaskService, fileMapper);
        when(aiUsageService.stats()).thenReturn(new AiUsageStatsVO());
        stubEmptyTaskAggregates();
        when(aiTaskService.taskStatTotals("text", "success")).thenReturn(taskTotals(2, 1, 100, 1));
        when(aiTaskService.taskStatRecent(eq("text"), any(), any(), any())).thenReturn(taskRecent(1, 2, 0));
        when(aiTaskService.taskStatTotals("image", "completed")).thenReturn(taskTotals(1, 1, 500, 1));
        when(aiTaskService.taskStatRecent(eq("image"), any(), any(), any())).thenReturn(taskRecent(1, 1, 0));
        when(aiTaskService.taskStatTotals("video", "completed")).thenReturn(taskTotals(1, 1, 1000, 1));
        when(aiTaskService.taskStatRecent(eq("video"), any(), any(), any())).thenReturn(taskRecent(1, 1, 0));
        when(aiTaskService.ttsStat(any())).thenReturn(ttsStat(1, 1, 0));
    ...
    void buildTaskStat_本周与上周边界() {
        ...
        when(aiUsageService.stats()).thenReturn(new AiUsageStatsVO());
        stubEmptyTaskAggregates();
        when(aiTaskService.taskStatTotals("text", "success")).thenReturn(taskTotals(4, 4, 0, 0));
        when(aiTaskService.taskStatRecent(eq("text"), any(), any(), any())).thenReturn(taskRecent(1, 2, 2));
    ...
    void stats_单模块异常_独立降级其余继续() {
        ...
        when(aiTaskService.taskStatTotals(anyString(), anyString())).thenThrow(new RuntimeException("task down"));
        when(aiTaskService.ttsStat(any())).thenThrow(new RuntimeException("task down"));
    ...
    void buildTaskStat_无终态_成功率0() {
        ...
        when(aiTaskService.taskStatTotals("text", "success")).thenReturn(taskTotals(0, 0, 0, 0));
        when(aiTaskService.taskStatRecent(eq("text"), any(), any(), any())).thenReturn(taskRecent(0, 0, 0));
