...
>
> **P11 已解决（聚合下推 SQL）**：仪表盘任务统计的 `ai_task` 全表拉取已改 `AiTaskMapper.selectTaskStatTotals`+`selectTaskStatRecent`（COUNT/SUM 下推 PG，`task_type` 过滤 + `request_time` 时间窗命中 `idx_ai_task_request_time`）+ `selectTtsStat`，不再整表进 JVM。故该项不再列入。

---

## 待优化点（代码层面的具体问题，共 7 项）
...
### 7. P17 — TTS 每片段新建 WebSocket 且超时未显式关闭（中 / 中）
...
1. **立即（高收益低风险）**：S1、P10（配置/清理，半天搞定）
2. **性能与稳定性（高优先级）**：P14（事务边界）→ P15（限流原子性）
