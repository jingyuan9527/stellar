...
- **文案/图片/视频三页统一布局**：均用 `views/video/components/AiGeneratorLayout.vue` 公共骨架——左 360px 配置栏（sticky）+ 右[结果/历史]双卡，`<1024px` 栈叠单列；三页只填 `aside/result/history` slots，替换原 `max-width:720px` 单列居中以利用横向空间。
- **文案工具 `/video/copy`**：登录态加载 TEXT 模型下拉（`getAiModelsByType('TEXT')`，默认选 isDefault），`streamAiChat` 第四参 opts 支持 `modelId`（选项目模型）/`endpoint+apiKey+model`（自带 key）/空（后端 TEXT 默认）。游客态仍可用自带 key 或后端默认模型。生成后右下常驻 `NDataTable` 历史（`getChatRecordPage`，后端流式结束自动落库 `sys_ai_chat_record`，登录按账号、游客按 IP），列=请求时间/提示词/结果/返回时间/耗时/操作，点击行或"查看"用 `parseCopyResult` 重新解析该条 `result` 展示到结果卡；游客也可见历史（按 IP）。不再调用 `sys_ai_copy_result`。
- **图片生成页 `/video/image`**：公开（`meta.requiresAuth: false`），Video 下子菜单（`icon: image`，order 3）。选 IMAGE 模型（`getAiModelsByType('IMAGE'`，接口 `@PublicAccess` 游客可查）+ prompt + size/ratio 下拉 → `POST /ai/image/create` 异步任务（`@RateLimit(daily=2)` 游客日限）返回 taskId → 轮询 `GET /ai/image/task/{id}`，completed 后 `NImage` 展示（url=`/file/{id}`，dev 代理）。生成后右下常驻 `NDataTable` 历史（`getAiImagePage` `@PublicAccess`，复用 `sys_ai_image_task`，按主体分页），列=请求时间/提示词/尺寸比例/结果(缩略图)/返回时间/耗时/操作，"查看"载入结果卡。游客历史按 IP 可见。
- **视频生成页 `/video/video`**：需登录，Video 下子菜单（`icon: play`，order 4）。选 VIDEO 模型 + prompt + ratio/duration 下拉 → `POST /ai/video/create` 异步任务（`@RateLimit(daily=3)` 日限）返回 videoId → 轮询 `GET /ai/video/status`（每 5s，含进度条），completed 后 `<video>` 播放（url=`/file/{id}`）。后端 `AiVideoService.createTask` 本地落库 `sys_ai_video_task`（存 ratio/duration 宽高帧率/videoId/subject），`getTask` 轮询时更新本地行 status/file_id（completed 复用本地 file_id 不重复下载）。生成后右下常驻 `NDataTable` 历史（`getAiVideoPage` 需登录，按账号分页），列=请求时间/提示词/比例时长/结果/返回时间/耗时/操作，"查看"载入视频。视频历史仅登录可见（生成本身需登录）。
