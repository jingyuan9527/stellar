...
- **文案工具 `/video/copy`**：登录态加载 TEXT 模型下拉（`getAiModelsByType('TEXT')`，默认选 isDefault），`streamAiChat` 第四参 opts 支持 `modelId`（选项目模型）/`endpoint+apiKey+model`（自带 key）/空（后端 TEXT 默认）。游客态仍可用自带 key 或后端默认模型。
- **图片生成页 `/video/image`**：公开（`meta.requiresAuth: false`），Video 下子菜单（`icon: image`，order 3）。选 IMAGE 模型（`getAiModelsByType('IMAGE'`，接口 `@PublicAccess` 游客可查）+ prompt + size 下拉 → `POST /ai/image/generate` 返回 `{fileId, url}` → `NImage` 展示（url=`/file/{id}`，dev 代理）。游客日限 2 次。
