...
- **神奇海螺页 `/game/conch`**：公开（`meta.requiresAuth: false`），Game 下子菜单（`icon: conch`，order 2）。海绵宝宝占卜道具玩法：输入框 + "拉绳"按钮 → `POST /tts/conch/ask` 返回 `{answerId, answerText, audioUrl}` → 展示回答文本（大号文字淡入）+ `getConchAnswerAudio` 拉音频 blob 自动播放（用户点击属手势，浏览器允许自动播放）+ 海螺 🐚 抖动动画（`asking` 时 `shaking` class）。后台 `system/conch`（`icon: conch`，order 8）用 `NTabs` 切"预设管理"（列表 + `NSwitch` 启停 + 试听 + `NModal` 增删改，`NUpload` custom-request 调 `uploadFile` 上传音频存 `sys_file` 拿 fileId）与"提问历史"两个 tab；AI 匹配开关改调 `GET/PUT /setting/conch_ai_enabled`（`SysSettingController`）。`api/conch.ts` 封装全部接口。
...
- **文案/图片/视频三页统一布局**：均用 `views/video/components/AiGeneratorLayout.vue` 公共骨架——左 360px 配置栏（sticky）+ 右历史单卡 + 内置 `NDrawer` 壳，`<1024px` 栈叠单列（抽屉全屏）；三页填 `aside/history/drawer` slots。历史点"查看"开右侧抽屉（`v-model:drawer-open` + `#drawer` slot）回看，去掉 result 卡、生成中按钮 loading + `message` 提示，完成后自动弹抽屉展示最新一条，结果全走历史/抽屉，避免历史结果撑高把历史卡顶出视口。替换原 `max-width:720px` 单列居中以利用横向空间。
- **文案工具 `/video/copy`**：登录态加载 TEXT 模型下拉（`getAiModelsByType('TEXT')`，默认选 isDefault），`streamAiChat` 第四参 opts 支持 `modelId`（选项目模型）/`endpoint+apiKey+model`（自带 key）/空（后端 TEXT 默认）。游客态仍可用自带 key 或后端默认模型。生成后右下常驻 `NDataTable` 历史（`getChatRecordPage`，后端流式结束自动落库 `sys_ai_chat_record`，登录按账号、游客按 IP），列=请求时间/提示词/结果/返回时间/耗时/操作，点击行或"查看"开右侧抽屉，用 `CopyResultDetail` 子组件（`parseCopyResult`）渲染该条 `result`；去掉 result 卡，生成中按钮 loading，完成后自动弹抽屉展示最新一条。游客也可见历史（按 IP）。不再调用 `sys_ai_copy_result`。
- **图片生成页 `/video/image`**：公开（`meta.requiresAuth: false`），Video 下子菜单（`icon: image`，order 3）。选 IMAGE 模型（`getAiModelsByType('IMAGE'`，接口 `@PublicAccess` 游客可查）+ prompt + size/ratio 下拉 → `POST /ai/image/create` 异步任务（`@RateLimit(daily=2)` 游客日限）返回 taskId → 轮询 `GET /ai/image/task/{id}`，completed 后自动弹抽屉用 `<img>` 展示大图+下载（url=`/file/{id}`，dev 代理）。生成后右下常驻 `NDataTable` 历史（`getAiImagePage` `@PublicAccess`，复用 `sys_ai_image_task`，按主体分页），列=请求时间/提示词/尺寸比例/结果(缩略图)/返回时间/耗时/操作，"查看"开右侧抽屉看大图+下载；去掉 result 卡，生成中按钮 loading，完成后自动弹抽屉展示最新一条。游客历史按 IP 可见。
- **视频生成页 `/video/video`**：需登录，Video 下子菜单（`icon: play`，order 4）。选 VIDEO 模型 + prompt + ratio/duration 下拉 → `POST /ai/video/create` 异步任务（`@RateLimit(daily=3)` 日限）返回 videoId → 轮询 `GET /ai/video/status`（每 5s），completed 后自动弹抽屉用 `<video>` 播放+下载（url=`/file/{id}`）。后端 `AiVideoService.createTask` 本地落库 `sys_ai_video_task`（存 ratio/duration 宽高帧率/videoId/subject），`getTask` 轮询时更新本地行 status/file_id（completed 复用本地 file_id 不重复下载）。生成后右下常驻 `NDataTable` 历史（`getAiVideoPage` 需登录，按账号分页），列=请求时间/提示词/比例时长/结果/返回时间/耗时/操作，"查看"开右侧抽屉播放+下载；去掉 result 卡，生成中按钮 loading，完成后自动弹抽屉展示最新一条。视频历史仅登录可见（生成本身需登录）。
